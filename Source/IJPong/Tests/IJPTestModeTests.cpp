...
bool FIJPTestModeResetTest::RunTest(const FString& Parameters)
{
	...
	UTEST_EQUAL("Left lost health", Mode->GetMatch()->GetHealth(EIJPSide::Left), 4.f);
	...
	Mode->RestartMatch();
	UTEST_EQUAL("Left back to full", Mode->GetMatch()->GetHealth(EIJPSide::Left), 5.f);
	UTEST_EQUAL("No goals yet", Mode->GetMatch()->GetGoals(EIJPSide::Right), 0);
...
bool FIJPTestModeOverlayTest::RunTest(const FString& Parameters)
{
	...
	UTEST_TRUE("Shows the match rules", Overlay().Contains(TEXT("Match: health 5 each")));
