...
- Screen look: **CRT post-process** (scanlines, gentle curvature + vignette, phosphor glow) via `UIJPCRTComponent`, which works on **any camera**, not just the arena's (the user wants it on menu cameras too). Material `M_CRT` (a Custom node that `#include`s `Shaders/IJPCRT.ush`), tuned per era in material instances (`MI_CRT_1972`, draft `MI_CRT_Arcade`).
