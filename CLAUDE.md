...
- Matches are run by **`UIJPMatchComponent`** on `AIJPGameModeBase` (`GetMatch()`): serve/score/win check, `OnMatchEnded(Winner)`; many matches can run in one level. Rules are **`UIJPMatchRules`** Data Assets (win target, 0 = endless; serve delay/angle). Match end is 1972 style: the ball stops and the winner's score blinks (`AIJPArena::ShowWinner`) until the next `StartMatch`.
...
- **Rivals**: `UIJPRival` (`/Game/Rivals`) = name + class + AI profile (style) + pre-match/win/loss lines. Difficulty is not the rival's: it stays the arena's `OpponentSkill`. `AIJPGameModeBase::SetRival` applies one live. The four rivals are placeholders ("<Class> Rival", no lines) for the user to name and write. Test mode: C cycles your class, V cycles the rival (including none).
