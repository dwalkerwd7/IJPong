...
- **Abilities** are custom (no GAS): `UIJPAbilityComponent` on each paddle has two slots (`ClassSkill`, `RunAbility`), each with a cooldown counted from activation. An ability is a C++ subclass of `UIJPAbility` (behaviour) plus Data Assets made from it (tuning, `/Game/Abilities`); equipping gives the slot its own copy (`DuplicateObject`) for runtime state. The class skill comes from the paddle profile's `ClassSkill` (`DA_Paddle_Default` = Smash); the run ability is `PlayerRunAbility` in the test mode's config (Grow) until runs exist. Keys: Space / gamepad A = class skill, E / gamepad B = run ability. The arena tells a paddle's abilities when it returns a ball (`HandleBallHit`). AI rivals don't use abilities yet.
