...
- Game modes subclass **`AIJPGameModeBase`** (finds the arena, player gets the left paddle, AI the right). **`AIJPTestGameMode`** is the project default for now: endless play of any level, with debug keys R (reset score), F (serve now), T (hand the player's paddle to an AI) and -/= (opponent skill). Real modes (match rules, chapters) come later as separate subclasses.
